...
public:
  Memset(Value &dst, Value &src, Value &bytes, unsigned align_dst, unsigned align_src)
    : Instr(Type::voidTy, "memset"), dst(dst), src(src), bytes(bytes), align_dst(align_dst), align_src(align_src) {}
